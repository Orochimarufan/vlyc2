...
class Video(vlyc.plugin.Video):
		...
		def xml(self, req, script_id=None):
			...
			if req == 'RpcApiSubtitle_GetXml':
				...
				data = {'req' : 'RpcApiVideoPlayer_GetStandardConfig', 'media_id' : self.videoId, 'video_format' : video_format, 'video_quality' : resolution, 'auto_play' : '1', 'show_pop_out_controls' : '1', 'current_page' : 'http://www.crunchyroll.com/'}
			else:
				data = {'req' : req, 'media_id' : self.videoId, 'video_format' : video_format, 'video_encode_quality' : resolution}

			res = self._httprequest(url, {"Referer": "http://static.ak.crunchyroll.com/flash/"+self.playerRev()+"/StandardVideoPlayer.swf"}, urllib.parse.urlencode(data))
			data = res.read()
